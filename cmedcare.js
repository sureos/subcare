...
let userID = '295ddf18-b06b-41f3-a1af-35ca5ce0820c';
				...
				case `/${userID}`: {
					...
					if ((!sub || sub == '') && (addresses.length + addressesapi.length + addressesnotls.length + addressesnotlsapi.length + addressescsv.length) == 0){
						if (request.headers.get('Host').includes(".workers.dev")) {
							...
							subconfig = env.SUBCONFIG || 'https://raw.githubusercontent.com/ACL4SSR/ACL4SSR/master/Clash/config/ACL4SSR_Online.ini';
						} else {
							sub = 'VLESS.fxxk.dedyn.io';
							subconfig = env.SUBCONFIG || "https://github.com/ACL4SSR/ACL4SSR/blob/master/Clash/config/ACL4SSR_Online_Full_MultiMode.ini";
						}
					}
